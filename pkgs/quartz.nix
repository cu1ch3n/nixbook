...
buildNpmPackage rec {
  ...
  dontNpmBuild = true;
  # Quartz imports esbuild at runtime even though upstream lists it in
  # devDependencies, so the default `npm prune --omit=dev` breaks the CLI.
  dontNpmPrune = true;

  doInstallCheck = true;
  installCheckPhase = ''
    runHook preInstallCheck
    "$out/bin/quartz" --help >/dev/null
